...
import { ArrowTrendingUpIcon, ArrowTrendingDownIcon, TrophyIcon, XCircleIcon } from '@heroicons/vue/24/outline';
...
<template>
    <li :class="[
        ...
        <div>
            <div>
                <label :for="`item_${index}`" class="sr-only">
                    Item {{ index }}
                </label>
                <div class="mt-2 flex rounded-md shadow-sm">
                    ...
                    <input type="text" v-model="item.text" :name="`item_${index}`" :id="`item_${index}`"
            ...
            <div class="mt-1 flex items-center gap-x-2">
                <div class="flex w-16 gap-x-2.5">
                    <div
                        ...
                        <TrophyIcon class="-ml-1 mr-0.5 h-5 w-5 flex-shrink-0 self-center text-gray-500"
                            aria-hidden="true" />
                        <span class="sr-only"> Item {{ index }} got choosen </span>
                        {{ item.count }}
                    </div>
                    <div
                        :class="[itemHasGoodOdds ? 'bg-green-400/10 text-green-400' : 'bg-red-400/10 text-red-400', 'inline-flex items-baseline rounded-full px-2.5 py-0.5 text-sm font-medium md:mt-2 lg:mt-0']">
        ...
        <dl class="flex w-full flex-none items-center justify-between gap-x-8 sm:w-auto">
            <div class="flex -space-x-0.5" title="Weight">
                ...
                <dd v-for="i in 5" :key="i">
                    ...
                    ]" @click="setItemWeight(i)"></button>
                </dd>
            </div>
            <div class="">
                <dt class="sr-only">Remove Item {{ index }}</dt>
                <dd>
                    <button class="bg-gray-900 rounded-full" @click="removeItem()">
